...
### 阶段 1：修复 API 调用 + 构建技能上下文

#### 可平衡的终态

1. **API 调用正常**
   - 配置 `minimax.api-key` 后能真实调用 MiniMax API
   - API 调用失败时返回 fallback 文本（非空方法体）

2. **技能上下文构建**
   - `buildContext()` 方法包含技能数据（从 `/api/skills` 查询）
   - 能回答"你会什么技术"类问题

3. **项目数据整合**
   - 项目列表按 sortOrder 排序取 Top 5
   - 返回项目名称、简介、技术栈
...
| 验证点 | 预期结果 |
|--------|----------|
| 请求 `/api/chat`，body: `{"question":"你会什么技术？"}` | 返回值包含 Java、Python、RISC-V 等技能关键词 |
| 请求 `/api/chat`，body: `{"question":"介绍一下你的项目"}` | 返回值包含项目名称和简介 |
| 请求 `/api/chat`，body: `{"question":"今天天气怎么样"}` | 返回值引导回技术话题（因不在知识库范围） |
| 无配置 api-key 时请求 | 返回 fallback 硬编码文本（不是报错） |

#### 关键约束

- **API URL**：使用 `https://api.minimax.chat/v1/text/chatcompletion_pro`（MiniMax API）
- **模型配置**：`minimax.model` 默认 `abab6.5s-chat`
- **fallback**：无 API Key 时返回预设文本，不抛出异常
- **不修改实体**：复用现有 Skill/Project 实体，不新增表
- **技能筛选**：只查已发布项目 `status = PUBLISHED`

#### 需修改的文件

| 文件 | 说明 |
|------|------|
| `ChatController.java` | 实现 `callAiApi()` 方法；注入 SkillRepository 构建技能上下文 |
...
| 验证点 | 预期结果 |
|--------|----------|
| 问"你会什么？" | 返回技能列表，非个人生活信息 |
| 问"今天天气怎么样" | 返回"我只回答技术问题哦~" 类引导语 |
| 问"你有女朋友吗" | 返回技术相关引导，不回答隐私问题 |
| 问项目相关 | 返回项目名称 + 简介 + 技术栈详细信息 |

#### 关键约束

- **Prompt 设计原则**：技术优先，简洁专业，礼貌拒绝
- **知识库内容**：Profile + Project + Skill + Article（只关联摘要）
- **不引入 RAG**：先用拼接字符串方案，简单可靠
- **中文回复**：全中文响应
...
| 文件 | 说明 |
|------|------|
| `ChatController.buildSystemPrompt()` | 重写 Prompt，定位技术顾问角色 |
...
2. **快捷问题按钮**
   - 显示 3-4 个推荐问题
   - 点击后自动发送并显示回答
   - 预设问题：你擅长什么、介绍一下你的项目、你用过哪些框架

3. **导航链接**
   - 导航栏"AI 对话"指向 `/chat.html`
   - 首页对话区域点击跳转
...
| 验证点 | 预期结果 |
|--------|----------|
| 访问 `/chat.html` | 显示对话界面，有输入框和快捷问题 |
| 点击快捷问题"你擅长什么" | 输入框填充问题，发送，得到回答 |
| 发送空白消息 | 不发送，提示"请输入问题" |
| 刷新页面 | 对话内容清除，重新开始 |
| 从首页点击"AI 对话" | 跳转 `/chat.html` |

#### 关键约束

- **样式统一**：复用 TechForge 现有配色和圆角风格
- **无 long-polling**：每次发送独立请求，不维护长连接
- **打字机效果**：可选，非强制
- **移动端适配**：输入框在底部，高度自适应
- **无 localStorage**：对话不持久化

#### 需修改的文件

| 文件 | 说明 |
|------|------|
| `static/chat.html` | 新建独立对话页面 |
| `static/index.html` | 对话区域添加跳转链接 |
...
- **超时时间**：10 秒后提示超时
- **防抖**：发送按钮点击后立即 disabled
- **错误提示**：用户友好，非技术error信息
- **保持 XSS**：用户输入仍需转义
...
将首页技能区域从静态改为动态，从 `/api/skills` API 加载技能数据。

### 可衡量的终态

1. 首页"技术栈"区域从 `/api/skills` 动态获取技能数据
2. 保留现有静态内容作为加载失败时的后备
3. 技能以卡片形式展示（与现有 `.skill-card` 样式一致）

### 明确的验证方式

| 验证点 | 预期结果 |
|--------|----------|
| 在管理端添加技能"Java" | 技能保存成功 |
| 刷新首页 | 技能区域显示"Java"卡片 |
| 删除技能"Java" | 刷新后"Java"消失 |
| API加载失败 | 显示原有静态技能作为后备 |
| 技能数量为0 | 显示"暂无技能" |

### 关键约束

- **API接口**：使用现有 `/api/skills` 接口
- **界面风格**：复用现有 `.skill-card` 样式
- **兼容性**：加载失败时显示原有静态内容
...
| 文件 | 说明 |
|------|------|
| `index.html` | 添加 loadSkills 函数，修改技能区域结构 |

### 实施步骤

1. 给技能区域容器添加 id（如 `id="skillsGrid"`）
2. 添加 `loadSkills()` 函数从 `/api/skills` 获取数据
3. 动态渲染技能卡片
4. DOMContentLoaded 时调用
...
## ChatController MiniMax API 修复目标

### 需求背景

AI 对话接口 `/api/chat` 调用 MiniMax API 时报错：
```
API 错误: invalid params, binding: expr_path=bot_setting, cause=missing required parameter
```

错误原因：MiniMax ChatCompletion Pro API 要求额外参数 `bot_setting`，当前代码只发送了 `model` 和 `messages`。

### 一、可平衡的终态

| 指标 | 目标值 |
|------|--------|
| API 调用成功率 | ≥ 95%（有效请求不报错） |
| 响应时间 | ≤ 10 秒 |
| 错误码识别 | 正确识别并返回友好错误信息 |
...
| 验证点 | 预期结果 |
|--------|----------|
| 请求 `POST /api/chat`，body: `{"question":"你好"}` | 返回正常的 AI 回复，非 API 错误 |
| 无 API Key 配置时请求 | 返回 fallback 预设文本，不报错 |
| 请求超额/限流时 | 返回友好的限流提示 |
| 配置错误时（如 model 不存在） | 返回配置错误友好提示 |

### 三、关键约束

| 约束类型 | 具体内容 |
|----------|----------|
| **API 参数** | 必须包含 `bot_setting` 参数（为空对象即可） |
| **兼容性** | 保持对不同模型的支持（model 可配置） |
| **错误处理** | 解析 MiniMax 的 `base_resp` 返回友好错误 |
| **降级** | 无 API Key 时返回 fallback 文本 |

### 四、MiniMax API 必传参数说明

根据 MiniMax ChatCompletion Pro 文档，请求体必须包含：

```json
{
  "model": "abab6.5s-chat",
  "messages": [...],
  "bot_setting": []  // 空数组即可，满足必传要求
}
```

### 五、完成状态

- [ ] 检查并修复 ChatController.callAiApi() 请求体
- [ ] 添加 `bot_setting` 参数
- [ ] 测试 API 调用正常返回